...
public:
    ...
    // move constructor
    Vector(Vector &&v) : elem_{v.elem_}, size_{v.size_} {
    ...
    // With the begin() and end() functions below, we can now iterate over our
    // container with range-for (see test2.cpp) Since these functions returns
    // the address for an internal data (resource handle), we need to have their
    // versions for const and non-const objects
